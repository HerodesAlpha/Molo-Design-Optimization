# 1 "D:/dev/MOLO_Design_Optimization/Fortran/Source/fNemoh/commonFiles/Results.f90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "D:\\dev\\MOLO_Design_Optimization\\build//"
# 1 "D:/dev/MOLO_Design_Optimization/Fortran/Source/fNemoh/commonFiles/Results.f90"
!--------------------------------------------------------------------------------------
!
...
! This module contains structure to hold a result
!
!   @author TCSASSEMBLER
!   @version 1.1
MODULE MResults
...
CONTAINS
    ...
    SUBROUTINE CreateTResults(Results,Nw,Nbeta,Nradiation,Nintegration,Ntheta)
        ...
        ALLOCATE(Results%w(Nw),Results%IndxForce(Nintegration,3),Results%beta(Nbeta),&
            &Results%IndxRadiation(Nintegration,3))
        ALLOCATE(Results%DiffractionForce(Nw,Nbeta,Nintegration),&
            &Results%FroudeKrylovForce(Nw,Nbeta,Nintegration),&
            Results%AddedMass(Nw,Nradiation,Nintegration),&
            &Results%RadiationDamping(Nw,Nradiation,Nintegration))
        Results%Ntheta=Ntheta
        IF (Results%Ntheta.GT.0) THEN            
            ALLOCATE(Results%Theta(Ntheta),Results%HKochinDiffraction(Nw,Nbeta,Ntheta),&
                Results%HKochinRadiation(Nw,Nradiation,Ntheta))
        END IF
    ...
    SUBROUTINE CopyTResults(ResultsTarget,ResultsSource)
        ...
        CALL CreateTResults(ResultsTarget,ResultsSource%Nw,ResultsSource%Nbeta,&
            ResultsSource%Nradiation,ResultsSource%Nintegration,ResultsSource%Ntheta)
    ...
    SUBROUTINE SaveTResults(Results,namedir)
        ...
        DO k=1,Results%Nintegration
            WRITE(10,'(A,I4,I4,A,I4,I4,A)') '"A',Results%IndxForce(k,2),&
                Results%IndxForce(k,3),'" "B',Results%IndxForce(k,2),Results%IndxForce(k,3),'"'
        END DO
        DO j=1,Results%Nradiation
            WRITE(10,'(A,I4,A,I4,A,I6,A)') 'Zone t="Motion of body ',Results%IndxRadiation(j,2),&
                &' in DoF',Results%IndxRadiation(j,3),'",I=',Results%Nw,',F=POINT'
            DO i=1,Results%Nw
                WRITE(10,'(80(1X,E14.7))') Results%w(i),(Results%AddedMass(i,j,k),&
                    Results%RadiationDamping(i,j,k),k=1,Results%Nintegration)
            END DO
        END DO
        CLOSE(10)
        ...
        WRITE(10,'(A)') 'VARIABLES="w (rad/s)"'
        DO k=1,Results%Nintegration
            WRITE(10,'(A,I4,I4,A,I4,I4,A)') '"abs(F',Results%IndxForce(k,2),&
                &Results%IndxForce(k,3),')" "angle(F',&
                Results%IndxForce(k,2),Results%IndxForce(k,3),')"'
        END DO
        DO j=1,Results%Nbeta
            WRITE(10,'(A,F7.3,A,I6,A)') 'Zone t="Diffraction force - beta = ',&
                Results%beta(j)*180./(4.*ATAN(1.0)),' deg",I=',Results%Nw,',F=POINT'
            DO i=1,Results%Nw
                WRITE(10,'(80(1X,E14.7))') Results%w(i),(ABS(Results%DiffractionForce(i,j,k)),&
                    ATAN2(AIMAG(Results%DiffractionForce(i,j,k)),&
                    REAL(Results%DiffractionForce(i,j,k))),k=1,Results%Nintegration)
            END DO
        END DO
        CLOSE(10)
        ...
        DO k=1,Results%Nintegration
            WRITE(10,'(A,I4,I4,A,I4,I4,A)') '"abs(F',Results%IndxForce(k,2),&
                Results%IndxForce(k,3),')" "angle(F',Results%IndxForce(k,2),Results%IndxForce(k,3),')"'
        END DO
        DO j=1,Results%Nbeta
            WRITE(10,'(A,F7.3,A,I6,A)') 'Zone t="Diffraction force - beta = ',&
                Results%beta(j)*180./(4.*ATAN(1.0)),' deg",I=',Results%Nw,',F=POINT'
            DO i=1,Results%Nw
                WRITE(10,'(80(1X,E14.7))') Results%w(i),&
                    &(ABS(Results%DiffractionForce(i,j,k)+Results%FroudeKrylovForce(i,j,k)),&
                    ATAN2(AIMAG(Results%DiffractionForce(i,j,k)+Results%FroudeKrylovForce(i,j,k)),&
                    REAL(Results%DiffractionForce(i,j,k)+Results%FroudeKrylovForce(i,j,k))),&
