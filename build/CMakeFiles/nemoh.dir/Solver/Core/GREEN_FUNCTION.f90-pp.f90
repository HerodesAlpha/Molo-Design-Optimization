# 1 "D:/dev/MOLO_Design_Optimization/Fortran/Source/fNemoh/Solver/Core/GREEN_FUNCTION.f90"
# 1 "<built-in>"
# 1 "<command-line>"
...
!   This module contains function to compute the value of green function and it's derivative
!   for the finite and infinite case. It compute the value for a given source point and a given field point
!   It is mostly re-using previous code.
!
!   Changes in version 1.1 (Dipoles Implementation in NEMOH)
...
CONTAINS
    ...
    SUBROUTINE VVV(deriv, KKK, J, XOI, YOI, ZOI, XGJ, YGJ, ZGJ, SolverVar)
        ! Compute the rankine part in infinite and finite depth
        ! Arguments:
        ! KKK: Id to indicate the form of the green function to use
        ! J: Panel index (not important here, any value should work)
        ! XOI, YOI,  ZOI: source point cartesian coordinate
        ! XGJ, YGJ, ZGJ: field point cartesian coordinate
        ! SolverVar: Common variables for the solver
        ...
        INTEGER :: KK(5)
        REAL :: XOI, YOI, ZOI, XOJ, YOJ, ZOJ, XGJ, YGJ, ZGJ
    ...
    SUBROUTINE VNV(deriv, J, XPO, YPO, ZPO, XGJ, YGJ, ZGJ, SolverVar, green, dgreen)
        ! Compute the value of the wave part of the green and it's derivative in infinite depth
        ! Arguments:
        ! J: Panel index (not important here, any value should ideally work)
        ! XOI, YOI,  ZOI: source point cartesian coordinate
        ! XGJ, YGJ, ZGJ: field point cartesian coordinate
        ! SolverVar: Common variables for the solver
        ...
        DO JJ = 1, NJJ
            ...
            YMJJJ = YGJ * BX
            QJJJ = YN(J) * BX
            RRR = SQRT((XPO - XGJ)**2 + (YPO - YMJJJ)**2)
            AKR = AK0 * RRR
            ZZZ = ZPO + ZGJ
            AKZ = AK0 * ZZZ
            DD = SQRT(RRR**2 + ZZZ**2)
        ...
        ELSE
            ...
            ! Divide by AIRE(J) to get the raw green function value
            ! This division should not be needed here as we are returning the raw value directly
            green = cmplx(FS1(J, 1) * 2, FS2(J, 1) * 2)
            tmp = XN(J) * VSX1(J, 1) + YN(J) * VSY1(J, 1) + ZN(J) * VSZ1(J, 1)
            dgreen = cmplx(tmp, XN(J) * VSX2(J, 1) + YN(J) * VSY2(J, 1) + ZN(J) * VSZ2(J, 1))
        ENDIF
        RETURN
    END SUBROUTINE
    !-------------------------------------------------------
    SUBROUTINE VNVF(deriv, AM0, AMH, NEXP, J, XPO, YPO, ZPO, XGJ, YGJ, ZGJ, SolverVar, green, dgreen)
        ! Compute the value of the wave part of the green and it's derivative in finite depth
        ! Arguments:
        ! AM0, AMH, NEXP: variables containing environment data: wave number, depth
        ! J: Panel index (not important here, any value should ideally work)
        ! XOI, YOI,  ZOI: source point cartesian coordinate
        ! XGJ, YGJ, ZGJ: field point cartesian coordinate
        ! SolverVar: Common variables for the solver
        ...
        ! It is only needed and supported currently when there is no symmetry around OxZ
        complex :: green, dgreen
        REAL :: AM0, XPO, YPO, ZPO, XGJ, YGJ, ZGJ
        REAL :: FS1(NFA, 2), FS2(NFA, 2)
        ...
        ELSE
            ...
            ! Divide by AIRE(J) to get the raw green function value
            ! The division should be check here or in the VVG method
            green = cmplx(FS1(J, 1) * 2, FS2(J, 1) * 2)
            tmp = XN(J) * VSX1(J, 1) + YN(J) * VSY1(J, 1) + ZN(J) * VSZ1(J, 1)
            dgreen = cmplx(tmp, XN(J) * VSX2(J, 1) + YN(J) * VSY2(J, 1) + ZN(J) * VSZ2(J, 1))
            !   491 CONTINUE
