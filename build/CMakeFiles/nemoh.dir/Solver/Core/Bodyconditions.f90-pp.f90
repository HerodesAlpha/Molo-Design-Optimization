# 1 "D:/dev/MOLO_Design_Optimization/Fortran/Source/fNemoh/Solver/Core/Bodyconditions.f90"
# 1 "<built-in>"
# 1 "<command-line>"
...
!   This module holds a structure defining the body conditions for a problem. It also contains
!   utilities to read write or create a body condition.
!
!   @author TCSASSEMBLER
!   @version 1.1
...
!
        ...
        ALLOCATE(BodyConditions%Omega(Nproblems),BodyConditions%Switch_Potential(Nproblems),&
                 BodyConditions%Switch_FreeSurface(Nproblems),&
                 BodyConditions%Switch_Kochin(Nproblems),&
                 BodyConditions%Switch_Type(Nproblems))
...
!       --- 
        ...
        IMPLICIT NONE
        TYPE(TBodyConditions) :: BodyConditions
        DEALLOCATE(BodyConditions%Omega,BodyConditions%Switch_potential,&
                   BodyConditions%Switch_FreeSurface,BodyConditions%Switch_Kochin,&
                   BodyConditions%Switch_Type)
