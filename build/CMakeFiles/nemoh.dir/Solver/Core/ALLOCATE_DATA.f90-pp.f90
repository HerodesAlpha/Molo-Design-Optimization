# 1 "D:/dev/MOLO_Design_Optimization/Fortran/Source/fNemoh/Solver/Core/ALLOCATE_DATA.f90"
# 1 "<built-in>"
# 1 "<command-line>"
...
!   This subroutine allocates variables used to solve all problems
!   Changes in version 1.2 (Code Acceleration of the Calculation of Influence Coefficients of Nemoh)
!       Allocate variables for caching rankine values.
!
!   @author yedtoss
!   @version 1.2
    ...
    ! DIST(I) is the maximum distance (projection on XY plane!) of a panel I from other points 
      ...
      ALLOCATE(XR(TABULATION_IR), XZ(TABULATION_JZ))
      ALLOCATE(APD1X(TABULATION_IR, TABULATION_JZ))
      ALLOCATE(APD1Z(TABULATION_IR, TABULATION_JZ))
      ALLOCATE(APD2X(TABULATION_IR, TABULATION_JZ))
      ALLOCATE(APD2Z(TABULATION_IR, TABULATION_JZ))

      ALLOCATE(rankine_sources_cache(NFA, NFA))
      ALLOCATE(rankine_dipoles_cache(NFA, NFA))
    !  
