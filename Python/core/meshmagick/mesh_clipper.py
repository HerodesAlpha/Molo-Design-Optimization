...
class MeshClipper:
    ...
    def _clip_crown_by_plane(self):
        ...
        inv_boundary_edges = dict(
            list(zip(new_id[list(inv_boundary_edges.keys())], new_id[list(inv_boundary_edges.values())])))
                ...
                if self._assert_closed_boundaries:
                    if len(open_lines) > 0:
                        try:
                            from . import mmio
                            mmio.write_VTP('full_debug.vtp', self.source_mesh.vertices, self.source_mesh.faces)
                            mmio.write_VTP('clipped_crown_debug.vtp', clipped_crown_mesh.vertices,
                                           clipped_crown_mesh.faces)
                            mmio.write_VTP('crown_debug.vtp', crown_mesh.vertices, crown_mesh.faces)
                        except:
                            pass
                        ...
                        raise RuntimeError(
                            'Open intersection curve found with assert_closed_boundaries option enabled. Files full_debug.vtp, crown_debug.vtp and clipped_crown_debug.vtp written.')
