...
class Mesh(object):
    ...
    def vertices(self, value):
        self._vertices = np.asarray(value, dtype=np.float64).copy()
    ...
    def faces(self, value):
        self._faces = np.asarray(value, dtype=np.int32).copy()
    ...
    def _faces_properties(self):
        ...
        faces_areas = np.zeros(nf, dtype=np.float64)
        faces_normals = np.zeros((nf, 3), dtype=np.float64)
        faces_centers = np.zeros((nf, 3), dtype=np.float64)
    ...
    def symmetrize(self, plane):
        ...
        vertices = np.concatenate(
            (vertices, vertices - 2 * np.outer(np.dot(vertices, plane.normal) - plane.c, plane.normal)))
    ...
    def de_symmetrize_xz(self):
        """de-Symmetrize the mesh with respect to a plane.
        ...
        # Symmetrizing the nodes
        vertices, faces = self._vertices, self._faces
        id_faces_to_extract = list()
        face_id = -1
        for vertex_ids in faces:
            face_id += 1
            y = 0
            for vertex_id in vertex_ids:
                y += vertices[vertex_id][1]
            if y > 0:  # Keep faces on positive side of xz-plane
                id_faces_to_extract.append(face_id)
        return self.extract_faces(id_faces_to_extract)
    ...
    def _compute_faces_integrals(self, sum_faces_contrib=False):  # TODO: implementer le sum_surface_contrib
        ...
        surface_integrals = np.zeros((15, self.nb_faces), dtype=np.float64)
    ...
    def eval_shell_mesh_inertias(self, rho_medium=7850., thickness=0.02):
        ...
        cog = np.array([s0, s1, s2], dtype=np.float64) / mass
    ...
    def _edges_stats(self):
        ...
        edge_length = np.zeros((self.nb_faces, 4), dtype=np.float64)
        for i in range(4):
    ...
    def _compute_triangles_integrals(triangles_vertices, sum_faces_contrib=False):
        ...
        s_int = np.zeros((15, triangles_vertices.shape[0]), dtype=np.float64)
        ...
        s_int[3] = delta * (
                6. * point_0.y * point_0.z + 3 * (point_1.y * point_1.z + point_2.y * point_2.z) - point_0.y * f1[:,
                                                                                                               2] - point_0.z * f1[
                                                                                                                                :,
                                                                                                                                1]) / 12.
        s_int[4] = delta * (
                6. * point_0.x * point_0.z + 3 * (point_1.x * point_1.z + point_2.x * point_2.z) - point_0.x * f1[:,
                                                                                                               2] - point_0.z * f1[
                                                                                                                                :,
                                                                                                                                0]) / 12.
        s_int[5] = delta * (
                6. * point_0.x * point_0.y + 3 * (point_1.x * point_1.y + point_2.x * point_2.y) - point_0.x * f1[:,
                                                                                                               1] - point_0.y * f1[
                                                                                                                                :,
                                                                                                                                0]) / 12.
