...
def simBody1DOF(time, Fex, Fdamp, dampType, M, Mainf, c, alpha, beta, moor=False, dof=[0, 0, 1, 0, 0, 0]):
    ...
    def fL(t, y, args):
        ...
        for iB in range(0, len(rhs) - 2):
            ...
            rhs[iB + 2] = args[iB + (len(args) - 5) / 2 + 5] * y[iB + 2] + args[iB + 5] * y[1]
        rhs[1] = eval(
            '(args[3]-args[-1]-args[4][1]*y[1]-(args[2]+args[4][2])*y[0]' + I + ')/(args[0]+args[1]+args[4][0])')
        return rhs
...
def simBodyReg1DOF(time, Fex, Fdamp, dampType, M, c, Ma, Bhyd, moor=False, dof=[0, 0, 1, 0, 0, 0]):
    ...
    def fL(t, y, args):
        rhs = [0] * 2
        rhs[0] = y[1]
        rhs[1] = (args[4] - args[-1] - (args[3] + args[5][1]) * y[1] - (args[1] + args[5][2]) * y[0]) / (
                    args[0] + args[2] + args[5][0])
        return rhs
...
def simBodyReg(time, Fex, Fdamp, dampType, M, c, Ma, B, moor=False, dof=[1, 1, 1, 1, 1, 1]):
    ...
    def f(t, y, args):
        ...
        rhs[4] = y[10]
        rhs[5] = y[11]
        rhs[6] = (args[27] - args[-6] - (args[6] + args[14]) * rhs[10] - args[16] * y[6] - args[22] * y[10]) / (
                    args[0] + args[8])
        rhs[7] = (args[28] - args[-5] - (args[7] + args[15]) * rhs[9] - args[17] * y[7] - args[23] * y[9]) / (
                    args[1] + args[9])
        rhs[8] = (args[29] - args[-4] - args[18] * y[8] - args[24] * y[2]) / (args[2] + args[10])
        rhs[9] = (args[30] - args[-3] - (args[7] + args[15]) * rhs[7] - args[19] * y[9] - args[23] * y[7] - args[25] *
                  y[3]) / (args[3] + args[11])
        rhs[10] = (args[31] - args[-2] - (args[6] + args[14]) * rhs[6] - args[20] * y[10] - args[22] * y[6] - args[26] *
                   y[4]) / (args[4] + args[12])
    ...
    while r.successful() and r.t < t1:
        ...
        if moor:
            if sum(dof) < 2:
                arg_f[dof.index(1) - 6] = 0.1 * ms.simLines(moorLib, xx[dof.index(1), iF], vv[dof.index(1), iF],
                                                            tim[iF], dt, dof)
            else:
