...
from pyNemoh import preprocessor
from pyNemoh import postprocessor
from pyNemoh import solver
import warnings
...
def generate_mesh(meshing_dir, params):
    ...
    try:
        ...
        with open(config_file_path, 'w') as f:
            f.write('\n'.join("%s: %s" % item for item in list(vars(params).items()) if item[1] is not None))
