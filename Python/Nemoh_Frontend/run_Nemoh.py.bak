...
def update_forces(idx, forces):
    ...
    f[0] = np.floor(idx / 3) + 1 # 08.05.2019 Eivind Sonju: Take floor of idx/3 to get correct index
    #f[0] = idx # TODO: Find out why index was set to i/3 +1
    f[idx % 3 + 1] = 1
    tmp = forces[-3:]
    f[-len(tmp):] = tmp
...
def generate_floating_bodies(simulation_parameter, simulation_dir):
    ...
    fbs = []
    for name, body in list(floating_bodies.items()):
...
def run_simulation(simulation_parameter, simulation_dir, queue):
    ...
    simulation_param = {
        ...
        'green_tabulation_simpson_npoints': jp.search(struct.H5_SOLVER_GREEN_TABULATION_SIMPSON_NPOINTS,
...
def run(user_config, queue):
    ...
    simulations = list(all_simulations.keys())
    ...
    for simulation in simulations:
        ...
        logger.info(
            'Running simulation ' + str(simulation) + ' with parameter ' + json.dumps(simulation_parameter) + '\n')
