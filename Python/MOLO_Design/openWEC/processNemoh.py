...
# def getPeriod(wdir):
#     pathFile = os.path.join(wdir, 'results', 'Fe.dat')
#     with open(pathFile, 'r') as f:
#         irfRaw = f.readlines()
...
def getAB(wdir, dof, nbody=1, sel=10):
    # Selected DOF
    if sum(dof) < 2:
        selDof = dof.index(1)
        # Open Radiation Coefficients    
        pathFile = os.path.join(wdir, 'results', 'RadiationCoefficients.tec')
        with open(pathFile, 'r') as f:
            irfRaw = f.readlines()
        ...
        for iA in range(sum(dof)):
            ...
            for iL in range(0, len(omega)):
                ...
                for iD in 2 * np.array(list(range(sum(dof)))):
                    i2 = indList[iD / 2]
                    A[i1, i2, iL] = dummy[iD]
...
#
# def getFe(wdir, dof, sel=10, nbody=1):
#     # Selected DOF
#     if sum(dof) < 2:
#         selDof = dof.index(1)
#         # Open Radiation Coefficients
#         pathFile = os.path.join(wdir, 'results', 'ExcitationForce.tec')
#         with open(pathFile, 'r') as f:
#             irfRaw = f.readlines()
#         strPat = 'I='
#         irfInd = 1 + sum(dof[0:selDof]) * 2
...
def getFe(wdir, dof, omega, dir, nbody=1):
    # Selected DOF
    if sum(dof) < 2:
        selDof = dof.index(1)
        # Open Radiation Coefficients
...
def getFS(wdir, advOps, depth, omeg, RAO):
    # Read Grid
    Nx = advOps['fsDeltaX']
    Ny = advOps['fsDeltaY']
    Lx = advOps['fsLengthX']
